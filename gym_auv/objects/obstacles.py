...
class CircularObstacle(BaseObstacle):
    ...
    def _calculate_boundary(self):
        return (
            shapely.geometry.Point(*self.position)
            .buffer(self.radius)
...
class VesselObstacle(BaseObstacle):
    ...
    def _calculate_boundary(self):
        ...
        boundary_temp = shapely.affinity.rotate(
            boundary_temp, ship_angle, use_radians=True, origin="centroid"
        )
        boundary_temp = shapely.affinity.translate(
            boundary_temp, xoff=self.position[0], yoff=self.position[1]
        )

...
def transform_body_to_ned(coordinates_body: np.ndarray, position_ned: np.ndarray, heading:float) -> np.ndarray:
    """Transforms an ndarray of shape (N, 2) array of body coordinates to NED coordinates"""
    rotation_matrix = Rz(heading)[:2,:2]
...
def Rz(psi):
    ...
    return np.vstack(
        [np.hstack([cpsi, -spsi, 0]), np.hstack([spsi, cpsi, -0]), np.hstack([0, 0, 1])]
    )
        ...
        [
            np.hstack(
                [
                    ...
                    cpsi * sphi - spsi * cphi * sth,
                ]
            ),
            np.hstack(
                [
                    cpsi * cth,
                    -spsi * cphi + sphi * sth * cpsi,
                    spsi * sphi + sth * cpsi * cphi,
                ]
            ),
            np.hstack([0, 0, 0]),
        ]

...
def report(env, report_dir, lastn=100):
    try:
        ...
        cross_track_errors = np.array(
            [obj["cross_track_error"] for obj in relevant_history]
        )
        ...
        speeds = np.array(
            [
                obj["pathlength"] / obj["duration"] if obj["duration"] > 0 else np.nan
                for obj in relevant_history
            ]
        )
        ...
        with open(os.path.join(report_dir, "report.txt"), "w") as f:
            ...
            f.write(
                "{:<30}{:<30.2f}\n".format(
                    "Avg. Cross-Track Error", cross_track_errors.mean()
                )
            )
        ...
        print(
            "Warning: Report files are open - could not update report: " + str(repr(e))
        )
    except OSError as e:
...
def plot_trajectory(
    env, fig_dir, local=False, size=100, fig_prefix="", episode_dict=None
):
    """
    ...
    for obst in obstacles:
        if not obst.static:
            ...
            if (not local) and isinstance(obst, VesselObstacle):
                ...
                ax.plot(
                    x_arr, y_arr, dashes=[6, 2], color="red", linewidth=0.5, alpha=0.3
                )
        ...
        if isinstance(obst, VesselObstacle):
            ...
            if local:
                ...
                vessel_obst_object = plt.Polygon(
                    np.array(list(vessel_obst.boundary.exterior.coords)),
                    True,
                    facecolor="#C0C0C0",
                    edgecolor="red",
                    linewidth=0.5,
    ...
    if episode_dict is None or local:
        ...
        if L + 5 >= SHADOW_LENGTH:
            ax.plot(
                path_taken[: L - SHADOW_LENGTH - 7, 0],
            ...
            ax.plot(
                path_taken[L - SHADOW_LENGTH + 7 + int(env.vessel.width) :, 0],
                path_taken[L - SHADOW_LENGTH + 7 + int(env.vessel.width) :, 1],
                dashes=[1 * dashmultiplier, 2 * dashmultiplier],
                color=pathcolor,
                linewidth=1.0 * linemultiplier,
                label=r"Path taken",
        ...
        normalize = mcolors.Normalize(
            vmin=min(episode_dict_colors), vmax=max(episode_dict_colors)
        )
    ...
    if not local:
        ax.annotate(
            ...
            family="sans-serif",
            bbox=dict(
                facecolor="tab:red", edgecolor="black", alpha=0.75, boxstyle="round"
            ),
        )
        ax.annotate(
            "Start",
...
def plot_scenario(env, fig_dir, fig_postfix="", show=True, also_save_png=True):
    ...
    for obst in env.obstacles:
        if isinstance(obst, CircularObstacle):
            ...
            obst_object = plt.Polygon(
                np.array(list(obst.boundary.exterior.coords)),
                True,
                facecolor="#C0C0C0",
                edgecolor="red",
                linewidth=0.5,
...
def plot_actions(
    env, agent, fig_dir, fig_prefix="", N=500, creategifs=True, createpdfs=True
):
    ...
    for i_t, theta in enumerate(theta_arr):
        for i_r, r in enumerate(r_arr):
            ...
            sys.stdout.write(
                "Plotting progress: {:.2%}\r".format((i_t * N + i_r + 1) / N**2)
            )
            sys.stdout.flush()
    print("\t" * 10)
    ...
    if creategifs:
        ...
        steer_surf = ax.plot_surface(
            Theta, R, steer_arr, cmap=cm.coolwarm, linewidth=0, antialiased=False
        )
        ...
        rot_animation.save(
            os.path.join(fig_dir, fig_prefix + "obst_avoidance_steer.gif"),
            dpi=80,
            writer="imagemagick",
        )
        ...
        fig = plt.figure()
        ax = fig.gca(projection="3d")
        surge_surf = ax.plot_surface(
            Theta, R, surge_arr, cmap=cm.coolwarm, linewidth=0, antialiased=False
        )
        ax.zaxis.set_major_formatter(FormatStrFormatter("%.02f"))
        ax.set_ylabel(r"Obstacle Distance")
        ax.set_xlabel(r"Relative Obstacle Angle")
        ...
        rot_animation.save(
            os.path.join(fig_dir, fig_prefix + "obst_avoidance_surge.gif"),
            dpi=80,
            writer="imagemagick",
        )
    ...
    if createpdfs:
        ...
        plt.savefig(
            os.path.join(fig_dir, fig_prefix + "policy_plot_thrust_contour.pdf")
        )
        ...
        plt.savefig(
            os.path.join(fig_dir, fig_prefix + "policy_plot_thrust_surface.pdf")
        )
        ...
        ax.yaxis.set_major_formatter(FuncFormatter(lambda y, _: "{:.0f} m".format(y)))
        plt.savefig(
            os.path.join(fig_dir, fig_prefix + "policy_plot_steering_contour.pdf")
        )
        ...
        plt.savefig(
            os.path.join(fig_dir, fig_prefix + "policy_plot_steering_surface.pdf")
        )
...
def plot_vector_field(
    ...
    for xidx, x in enumerate(X):
        for yidx, y in enumerate(Y):
            ...
            sys.stdout.write(
                "Simulating behavior {:.2%} ({}/{})\r".format(
    ...
    plt.rc("axes", labelsize=8)
    ax.plot(
        path[0, :], path[1, :], dashes=[6, 2], linewidth=1.0, color="red", label=r"Path"
    )
    ax.set_ylabel(r"North (m)")
    ax.set_xlabel(r"East (m)")
    ax.xaxis.set_major_formatter(FuncFormatter(lambda y, _: "{:.0f}".format(y * 10)))
...
def test_report(fig_dir):
    ...
    for episode in range(1000):
        ...
        timesteps_baseline = env.path.length / (
            env.config["cruise_speed"] * env.config["t_step_size"]
        )
        env.history.append(
            {
                ...
                "reward": np.random.normal(
                    -1000 + 2000 * progress + episode**1.1, 2000
                ),
                "timesteps": (np.random.random() + 0.1)
                * np.random.poisson(timesteps_baseline + progress * 500 + episode),
    ...
    env.last_episode = {"path": path, "path_taken": path_taken, "obstacles": []}
    env.obstacles = []
    for _ in range(np.random.poisson(10)):
        obst = Struct()
        s = int(1000 * np.clip(np.random.random(), 0.1, 0.9))
        obst.position = np.array([path_x[s], path_y[s]])

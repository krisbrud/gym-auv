...
class TestScenario2(BaseEnvironment):
    def _generate(self):
        ...
        while True:
            ...
            obst_position = self.path(obst_arclength)
            obst_displacement_angle = (
                self.path.get_direction(obst_arclength) - np.pi / 2
            )
            obst_displacement = obst_displacement_dist * np.array(
                [np.cos(obst_displacement_angle), np.sin(obst_displacement_angle)]
            )

            self.obstacles.append(
                CircularObstacle(obst_position + obst_displacement, obst_radius)
            )
            self.obstacles.append(
                CircularObstacle(obst_position - obst_displacement, obst_radius)
            )
...
class TestHeadOn(BaseEnvironment):
    def _generate(self):
        ...
        trajectory_shift = (
            5 * deg2rad
        )  # random.uniform(-5*deg2rad+start_angle, 5*deg2rad+start_angle) #2*np.pi*(rng.rand() - 0.5)
...
class TestCrossing(BaseEnvironment):
    def _generate(self):
        ...
        trajectory_shift = (
            90 * deg2rad
        )  # random.uniform(-5*deg2rad, 5*deg2rad) #2*np.pi*(rng.rand() - 0.5)
            ...
            for i in range(10000):
                other_vessel_trajectory.append(
                    (
                        i,
                        (
                            245 + 2.5 * vessel_idx + trajectory_shift,

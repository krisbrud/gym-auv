    ...
    def _generate(self):
        ...
        if self.vessel_data_path is not None:
            ...
            while len(vessel_trajectories) < self.n_vessels:
                ...
                vessels[vessel_name]["AIS_Timestamp"] -= vessels[vessel_name].iloc[0][
                    "AIS_Timestamp"
                ]
                ...
                for _, row in vessels[vessel_name].iterrows():
                    ...
                    if timedelta < cutoff_dt:
                        if last_east is not None:
                            ...
                            if (
                                speed < VESSEL_SPEED_RANGE_LOWER
                                or speed > VESSEL_SPEED_RANGE_UPPER
                            ):
                                path = []
                                continue
                        ...
                        path.append(
                            (
                                int(
                                    (
                                        row["AIS_Timestamp"] - start_timestamp
                                    ).total_seconds()
                                ),
                        ...
                        if (
                            len(path) > 1
                            and not np.isnan(row["AIS_Length_Overall"])
        ...
        other_vessel_indeces = self.rng.choice(
            list(range(len(vessel_trajectories))),
            min(len(vessel_trajectories), self.n_vessels),
            replace=False,
        )
        ...
        if self.verbose:
            print("Generating {} vessel trajectories".format(len(self.other_vessels)))
        for vessel_width, vessel_trajectory, vessel_name in self.other_vessels:
            ...
            if len(vessel_trajectory) > 2:
                vessel_obstacle = VesselObstacle(
                    width=int(vessel_width),
    ...
    def _update(self, force=False):
        if self.renderer == "3d":
            if self.t_step % UPDATE_WAIT == 0 or force:
                travelled_distance = (
                    np.linalg.norm(
                        self.vessel.position - self.last_scenario_load_coordinates
                    )
                    if self.last_scenario_load_coordinates is not None
                    else np.inf
                )
                if travelled_distance > self.render_distance / 10:
                    if self.verbose:
                        print(
                            "Update scheduled with distance travelled {:.2f}.".format(
                                travelled_distance
                            )
                        )
                    ...
                    for obstacle in self.all_obstacles:
                        obst_dist = (
                            float(vessel_center.distance(obstacle.boundary))
                            - self.vessel.width
                        )
                        if obst_dist <= self.render_distance:
                            ...
                            if not obstacle.static:
                                obstacle.update(
                                    UPDATE_WAIT * self.config.simulation.t_step_size
                                )

                    if self.verbose:
                        print(
                            "Loaded nearby terrain ({} obstacles).".format(
                                len(self.obstacles)
                            )
                        )
                    ...
                    self._viewer3d.create_world(
                        self.all_terrain, xlow, ylow, xhigh, yhigh, self.x0, self.y0
                    )
                    if self.verbose:
                        print(
                            "Loaded nearby 3D terrain ({}-{}, {}-{})".format(
...
class Sorbuoya(RealWorldEnv):
    ...
    def _generate(self):
        ...
        self.path = Path(
            [[1000, 830, 700, 960, 1080, 1125], [910, 800, 700, 550, 750, 810]]
        )
        self.obstacle_perimeters = np.load("../resources/obstacles_sorbuoya.npy")
        self.all_terrain = (
            np.load(TERRAIN_DATA_PATH) / 7.5
        )  # np.load(TERRAIN_DATA_PATH)[0000:2000, 10000:12000]/7.5
        super()._generate()
...
class Trondheim(RealWorldEnv):
    ...
    def _generate(self):
        self.path = Path(
            [[6945 - self.x0, 6329 - self.x0], [4254 - self.y0, 5614 - self.y0]]
        )
        self.obstacle_perimeters = np.load("../resources/obstacles_trondheim.npy")
        self.all_terrain = (
            np.load(TERRAIN_DATA_PATH)[self.x0 : 8000, self.y0 : 6900] / 7.5
        )
        super()._generate()
...
class Trondheimsfjorden(RealWorldEnv):
    ...
    def _generate(self):
        self.path = Path(
            [
                [520, 1070, 4080, 5473, 10170, 12220],
                [3330, 5740, 7110, 4560, 7360, 11390],
            ]

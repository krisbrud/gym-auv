...
def _render_progress(env):
    ...
    target_point = env.path(
        env.vessel._last_navi_state_dict["target_arclength"]
    ).flatten()
    env._viewer2d.draw_circle(
        origin=target_point, radius=1, res=30, color=(0.3, 0.8, 0.3)
    )
...
def render_env(env, mode):
    ...
    if DYNAMIC_ZOOM:
        if int(env.t_step / 1000) % 2 == 0:
            env._viewer2d.camera_zoom = 0.999 * env._viewer2d.camera_zoom + 0.001 * (
                ZOOM - env._viewer2d.camera_zoom
            )
        else:
            env._viewer2d.camera_zoom = 0.999 * env._viewer2d.camera_zoom + 0.001 * (
                1 - env._viewer2d.camera_zoom
            )

    env._viewer2d.transform.set_scale(
        env._viewer2d.camera_zoom, env._viewer2d.camera_zoom
    )
    env._viewer2d.transform.set_translation(
        WINDOW_W / 2
        - (
            scroll_x * env._viewer2d.camera_zoom * cos(rot_angle)
            - scroll_y * env._viewer2d.camera_zoom * sin(rot_angle)
        ),
        WINDOW_H / 2
        - (
            scroll_x * env._viewer2d.camera_zoom * sin(rot_angle)
            + scroll_y * env._viewer2d.camera_zoom * cos(rot_angle)
        ),
    )

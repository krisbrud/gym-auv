...
pywavefront.configure_logging(
    logging.FATAL, formatter=logging.Formatter("%(name)s-%(levelname)s: %(message)s")
)
...
def plane_vertices(x, z, y_00, y_01, y_10, y_11, n):
    return [
        x - n,
        y_00,
        z - n,
...
def get_neighbours(x, y, matrix, include_diag=False):
    ...
    for dx in (-1, 0, 1):
        for dy in (-1, 0, 1):
            ...
            if (
                p[0] >= 0
                and p[0] < matrix.shape[0]
                and p[1] >= 0
                and p[1] < matrix.shape[1]
            ):
                ans.add(p)
...
class Viewer3D(object):
    ...
    def create_world(self, terrain, xlow, ylow, xhigh, yhigh, xoffset=0, yoffset=0):
        ...
        for x_terrain in range(xlow, xhigh):
            for y_terrain in range(ylow, yhigh):
                ...
                if (
                    x_terrain == xlow
                    or x_terrain == (xhigh - 1)
                    or y_terrain == ylow
                    or y_terrain == (yhigh - 1)
                ):
                    ...
                    h_00 = np.mean(
                        [z, terrain[x - 1][y], terrain[x - 1][y - 1], terrain[x][y - 1]]
                    )
                    h_01 = np.mean(
                        [z, terrain[x - 1][y], terrain[x - 1][y + 1], terrain[x][y + 1]]
                    )
    ...
    def _show_element(self, position, texture, shape, element_type):
        ...
        if element_type == Element.BLOCK:
            vertex_data = cube_vertices(
                x, z, shape[0], shape[1], shape[2], shape[3], 0.5
            )
            self._shown[position] = self.main_batch.add(
                24,
                GL_QUADS,
                self.group,
            ...
            vertex_data = plane_vertices(
                x, z, shape[0], shape[1], shape[2], shape[3], 0.1
            )
            self._shown[position] = self.overlay_batch.add(
                4,
                GL_QUADS,
                self.group,
                ("v3f/static", vertex_data),
    ...
    def draw_label(self, env):
        ...
        self.label.text = "Lg. λ: {:.2f}".format(
...
def render_env(env, mode, dt):
    x, y, z = (
        env.vessel.position[1],
        env._viewer3d.camera_height,
        env.vessel.position[0],
    )
    ...
    try:
        camera_terrain_height = env.all_terrain[int(camera_z + env._viewer3d.xoffset)][
            int(camera_x + env._viewer3d.yoffset)
        ]
    except IndexError:
    ...
    if env._viewer3d.autocamera:
        ...
        if abs(d_height) < 1 or abs(d_distance) < 1:
            env._viewer3d._reset_moving_camera(
                init_angle=-env.vessel.heading * 180 / np.pi + 180
            )
        ...
        env._viewer3d.camera_angle += (
            dt
            * CAMERA_ROTATION_SPEED
            * 180
            / np.pi
            * geom.princip(d_angle * np.pi / 180)
        )
        ...
        if env._viewer3d.camera_angle is None:
            ...
            env._viewer3d.camera_angle += (
                ...
                * geom.princip(
                    -env.vessel.heading
                    + np.pi
                    - env._viewer3d.camera_angle * np.pi / 180
                )
            )

    env._viewer3d.position = (camera_x, camera_y, camera_z)
    env._viewer3d.rotation = (
        env._viewer3d.camera_angle,
        -180 / np.pi * np.arctan2(y, env._viewer3d.camera_distance),
    )
    ...
    for obsvessel in vessel_obstacles:
        ...
        visualization.draw(
            env._viewer3d.boat_models[(VESSEL_MODEL_PATH, obsvessel.width)]
        )
        glRotatef(-90 - obsvessel.heading * 180 / np.pi, 0, 0, 1)
        glRotatef(90, 1, 0, 0)
        glTranslatef(-obsvessel.position[1], 1, -obsvessel.position[0])
    ...
    visualization.draw(
        env._viewer3d.boat_models[(TUGBOAT_MODEL_PATH, env.vessel.width)]
    )
    glRotatef(-90 - env.vessel.heading * 180 / np.pi, 0, 0, 1)
    glRotatef(90, 1, 0, 0)
    glTranslatef(-x, -0.3, -z)
